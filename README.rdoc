= Ruby on Rails 郵便番号検索 gem JpAddress: A Rails plugin for Japan Zipcode API.

== JpAddressとは
日本郵便の「郵便番号データ」を Rails 6.1 で使用するための gem です。
以下の機能を提供します。

* 郵便局提供の郵便番号データをダウンロードしてＤＢにロードするクラスメソッド。（JpAddress::Zipcode.load_master_data）
* 郵便番号をキーに住所情報を返却するＡＰＩ（jp_address/zipcodes#search）。
